...
GPTclient = OpenAI(api_key=os.environ["OPENAI_API_KEY"])
...
def api_call(prompt, deployment_name, temperature, max_tokens, top_p):
    ...
    """  # Change to avoid rate limit
    if deployment_name in ["gpt-35-turbo", "gpt-4", "gpt-3.5-turbo", "gpt-4o"]:
        time.sleep(4)
    ...
    elif deployment_name in ["llama-3-8b", "llama-3-70b"]:
        model_name = ""
