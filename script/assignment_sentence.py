...
def assign_topics(
    ...
    for i in trange(len(docs)):
        ...
        sentences_json = [{"id": i, "text": sentence} for i, sentence in enumerate(sentences, start=1)]
        sentences_response = []
        for j in sentences_json:
            cos_sim = {}
            doc_emb = sbert.encode(doc, convert_to_tensor=True)
            sentence_emb = sbert.encode(j['text'], convert_to_tensor=True)
            ...
            max_doc_len = (
                ...
                - num_tokens_from_messages(j['text'], deployment_name)
            )
            ...
            try:
                prompt = assignment_prompt.format(Document=doc, tree=seed_str,sentence=j['text'])
                # with open("my_prompt2/Example_prompts/"+str(tt)+"_assignment.txt","w") as f:
                #     f.write(prompt)
                #     tt +=1
                ...
                sentences_response.append(result_json)
                ...
                sentences_response.append(result_json)
                traceback.print_exc()
        prompted_docs.append(sentences)
        res.append(sentences_response)
